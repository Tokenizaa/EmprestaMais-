...
export const Documentos: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      supabaseService.db.getDocuments(user.id).then(setDocuments).catch(console.error);
    }
  }, [user]);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Gamification: Reward user for uploading documents
      await addPoints(20, 'Envio de Documento');
      
      alert('Documento enviado com sucesso! Você ganhou 20 pontos XP.');
              ...
              <CardContent>
                  <form onSubmit={handleUpload} className="space-y-4">
                      ...
                      <div className="border-2 border-dashed border-slate-700 rounded-lg p-6 text-center hover:bg-slate-800/50 transition-colors cursor-pointer relative group">
                          ...
                          />
                          <Upload className="mx-auto h-10 w-10 text-slate-500 mb-2 group-hover:text-primary transition-colors" />
                  ...
                  <Card key={doc.id} variant="glass">
                      <CardContent className="p-4 flex items-center justify-between">
                          <div className="flex items-center gap-4">
                              <div className="bg-slate-800 p-3 rounded-lg text-indigo-400 border border-slate-700">
                                  <FileText size={24} />
                              </div>
                              <div>
                                  <h3 className="font-bold text-white">{doc.tipo}</h3>
                                  <p className="text-xs text-slate-400 truncate max-w-[150px] sm:max-w-none">{doc.nome_arquivo}</p>
                                  <p className="text-xs text-slate-500">{new Date(doc.data_envio).toLocaleDateString()}</p>
                              </div>
                          </div>
                          <div>
