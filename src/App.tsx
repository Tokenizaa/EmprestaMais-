...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem('emprestamais_session_user');
    if (storedUser) {
  ...
  const login = async (email: string, pass: string) => {
    ...
    setUser(loggedUser);
    localStorage.setItem('emprestamais_session_user', JSON.stringify(loggedUser));
  };

  const logout = () => {
    setUser(null);
    localStorage.removeItem('emprestamais_session_user');
  };

  const refreshUser = async () => {
     if(!user) return;
     const { user: updated, error } = await supabaseService.auth.signIn(user.email);
     if(updated) {
       setUser(updated);
       localStorage.setItem('emprestamais_session_user', JSON.stringify(updated));
     }
  };
...
// Rota Protegida com Layout Específico e Role Check
