...
export interface DadosProfissionais {
  ...
  empresa: string;
  salario: number;
  tempo_empresa: string; // ex: "2 anos"
}
...
export interface DadosFinanceiros {
  ...
  possui_conta_corrente: boolean;
  possui_cartao_credito: boolean;
}
...
export interface LoanOffer {
  id: string;
  lender_id: string; // Quem empresta
  lender_name: string;
  amount: number;
  interest_rate: number; // Taxa mensal
  months: number;
  description: string;
  created_at: string;
}
...
export interface LoanContract {
  id: string;
  offer_id: string;
  borrower_id: string; // Quem pega emprestado
  lender_id: string; // Quem emprestou
  borrower_name: string;
  status: LoanStatus;
  total_amount: number;
